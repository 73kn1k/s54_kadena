<?php
  ...
  if (isset($_GET['e']) && $_GET['e'] == 1 && isset($_POST['stripeToken']) && !empty($_POST['stripeToken'])) {
    ...
    try {
      ...
      $_url = "http://127.0.0.1:3000/createToken?ntw=$kdaNtw&chainId=$chainId&collection_id=".urlencode( stored_collection_id )."&customer_pubK=". userSessionKDAwallet ."&store_pubK=". olimpiaKDAwallet ."&uri=".urlencode($_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].substr($newFileNameJSON,1));
      $ch = curl_init();
